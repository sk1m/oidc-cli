...
func receiveCodeViaLocalServer(ctx context.Context, opts *AuthCodeFlowOpts) (string, error) {
	l, err := NewListener(opts.LocalServerBindAddress)
	if err != nil {
		return "", fmt.Errorf("could not start a local server: %w", err)
	}
	defer l.Close()

	opts.OAuth2Config.RedirectURL = computeRedirectURL(l, opts)

	respCh := make(chan *authorizationResponse)
	server := http.Server{
		Handler: opts.LocalServerMiddleware(&localServerHandler{
			config: opts,
			respCh: respCh,
		}),
	}
	...
	g.Go(func() error {
		...
		opts.Logf("oidc.server: starting a server at %s", l.Addr())
		defer opts.Logf("oidc.server: stopped the server")
	...
	g.Go(func() error {
		...
		opts.Logf("oidc.server: shutting down the server")
		ctx, cancel := context.WithTimeout(ctx, 500*time.Millisecond)
		defer cancel()
		if err := server.Shutdown(ctx); err != nil {
			opts.Logf("oidc.server: force-closing the server: shutdown failed: %s", err)
			_ = server.Close()
			return nil
		}
		return nil
	...
	g.Go(func() error {
		if opts.LocalServerReadyChan == nil {
			return nil
		}
		select {
		case opts.LocalServerReadyChan <- opts.OAuth2Config.RedirectURL:
			return nil
		case <-ctx.Done():
			return ctx.Err()
		}
...
func computeRedirectURL(l net.Listener, opts *AuthCodeFlowOpts) string {
	hostPort := fmt.Sprintf("%s:%d", opts.RedirectURLHostname, l.Addr().(*net.TCPAddr).Port)
...
func (h *localServerHandler) handleIndex(w http.ResponseWriter, r *http.Request) {
	...
	h.config.Logf("oidc.server: sending redirect to %s", authCodeURL)
...
func (h *localServerHandler) handleCodeResponse(w http.ResponseWriter, r *http.Request) *authorizationResponse {
	...
	if state != h.config.State {
		...
		return &authorizationResponse{
			err: fmt.Errorf("state does not match (wants %s but got %s)", h.config.State, state),
		}
	}
	...
	if h.config.SuccessRedirectURL != "" {
		...
		if _, err := fmt.Fprintf(w, h.config.LocalServerSuccessHTML); err != nil { // nolint:govet
